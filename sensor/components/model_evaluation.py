...
#Idea is to compare the existing "model.pkl" file running in prod (if availabe) with the
#newly created file in Training Pipeline and the one with better accuracy will be used in Prod 

#For Model to be used in Prod we will need
# model.pkl file
#transformer.pkl file
#target_encoder.pkl file
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)->artifact_entity.ModelEvaluationArtifact:
        ...
        # which model is best trained or the model from saved model folder
            logging.info(f"if saved model folder has model then we will compare which model "
            "is best trained or the model from the saved folder")
            ...
            #Finding locations of transformers model and target encoder
            logging.info("Finding locations of transformers model and target encoder")
            transformer_path = self.model_resolver.get_latest_transformer_path()
            model_path = self.model_resolver.get_latest_save_model()
            ...
            current_transformers = load_object(file_path = self.data_transformation_artifact.transform_object_path)
            current_model = load_object(file_path = self.model_trainer_artifact.model_path)
            current_target_encoder = load_object(file_path = self.data_transformation_artifact.target_encoder_path)



            #we will do the comparison of both the models on test df
            ...
            #Converting the encoded values from 0/1 to yes/no
