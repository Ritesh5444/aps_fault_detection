...
class modeltrainer:
    ...
    def __init(self, model_trainer_config: config_entity.ModelTrainerConfig,
    ...
    def initiate_model_trainer(self,)-> artifact_entity.ModelTrainerArtifact:
        ...
        try:
            logging.info(f"Loading train adn test array")
            train_arr = utils.load_numpy_array_data(file_path = self.data_transformation_artifact.transformed_train_path)
            test_arr = utils.load_numpy_array_data(file_path = self.data_transformation_artifact.transformed_test_path)
    
            logging.info(f"splitting input and target feature from both train and test arr")
            x_train, y_train = train_arr[:,:-1], train[:,-1]
            x_test, y_test = test_arr[:,:-1], test[:,-1]
            ...
            logging.info("Checking if our model is overfititng or not")
            ...
            logging.info(f"Prepare the object")
            utils.save_object(file_path = self.model_trainer_config.model_path, obj = model)

            #prepare artifact
            logging.info(f"Prepare the artifact")
            model_trainer_artifact = artifact_entity.ModelTrainerArtifact(model_path = self.self.model_trainer_config.model_path,
